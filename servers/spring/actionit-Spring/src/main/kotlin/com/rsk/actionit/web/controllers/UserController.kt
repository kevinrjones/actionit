...
class UserController(var userService: ActionItUserService, var log: Log) {
    ...
    fun user(principal: Principal?): User {
        if(principal == null) {
            ...
            val user = (principal as UsernamePasswordAuthenticationToken).principal as AuthenticatedUser
    ...
    fun createUser(@RequestBody user: RegistrationUser):  ResponseEntity<User> {
        if(user.email.isNotEmpty()
            ...
            val authenticatedUser = userService.registerNewUserAccount(ActionItUser(user.email, user.firstName, user.lastName, user.password, user.repeatPassword))
